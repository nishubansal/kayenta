  ...
  boolean registerAzureCredentials(AzureConfigurationProperties azureConfigurationProperties,
    ...
    for (AzureManagedAccount azureManagedAccount : azureAccounts) {
      ...
      try {
        ...
        if (!CollectionUtils.isEmpty(supportedTypes)) {
          if (supportedTypes.contains(AccountCredentials.Type.OBJECT_STORE)) {
            ...
            if (StringUtils.isEmpty(container)) {
              throw new IllegalArgumentException("Azure/Blobs account " + name + " is required to specify a container.");
            }
        ...
        AzureNamedAccountCredentials azureNamedAccountCredentials = azureNamedAccountCredentialsBuilder.build();
